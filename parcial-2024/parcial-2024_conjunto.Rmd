...
ggplot(ascorbate_log_long, aes(x = type, y = survival)) + 
  ...
  geom_boxplot() + 
  theme_minimal() +
  labs(title = "Distribución de días de supervivencia para\npacientes con cáncer de mama y de estómago",
       x = "Tipo de cáncer",
       y = "Días de supervivencia (log)")

```

Ahora realizamos las permutaciones
...
Es evidente qué la gráfica de caja y brazos que pertenece a la muestra original es la gráfica 10, entonces podemos decir que rechazamos la hipotesis nula de que ambas distribuciones (cancer de mama y de estomago) provienen de la misma distribucion.
...
Hipotesis nula: No hay diferencia en la media de los tiempos de supervivencia 
Hipotesis alterna: Hay una diferencia en la media de los tiempos de supervivencia entre los grupos de 	 	cancer de mama y cancer de estomago.

Primero revisamos nuestra tabla de permutaciones
```{r}
...
Despues mostramos nuestros valores de referencia
```{r}
...
Graficamos la distribucion de nuestros valores de referencia
...
g_1 <- ggplot(valores_ref, aes(sample = diferencia)) + geom_qq(distribution = stats::qunif)  +
  ...
  annotate("text", x = 0.18, y = dif_obs + 0.30, label = "diferencia\nobservada", colour = "red")
...
Observamos nuestro valor p de una y dos colas (el que nos interesa es el de dos colas). Debido a que el valor p de la prueba de dos colas es menor a 0.05, rechazamos la hipotesis nula y concluimos que hay una diferencia en la media de los tiempos de supervivencia entre los grupos de cancer de mama y cancer de estomago.

```{r}
valor_p = 1 - percentil_obs
sprintf("Valor p de la prueba de una cola: %1.5f",round(valor_p,3))

valor_p_dos_colas = 2 * min(dist_perm(dif_obs), (1 - dist_perm(dif_obs)))
sprintf("Valor p de la prueba de dos colas: %1.5f",round(valor_p_dos_colas,3))
#valor_p_dos_colas
```
...
Podemos ver en las graficas que para aquellos sujetos donde hay valores de prueba y referencia la concentración de zolmitriptan es similar para la mayoría, salvo para los sujetos 6, 10 y 18. También podemos observar que para la mayoríoa de los individuos, la concentración en la sangre es practicamente cero después de la hora 10 después de la administración de la dosis, tanto para la formulación de referencia como la de prueba.
...
# Realizamos una gráfica por sujeto, y para cada una de las formulaciones disponibles
ggplot(zolmitriptan, aes(x = hora, y = concentracion, group = formulation, colour = formulation)) +
  geom_line() + 
  labs(title="Concentración de Zolmitriptan en la sangre para diversos individuos",
       x="Tiempo transcurrido desde la toma de la dosis (horas)", 
       y ="Concentración de zolmitriptan (mCG/ml)",
       colour ="") +
...
Presentamos una tabla generada con gt.

```{r}
# Creamos una tabla llamada auc que contiene el área bajo la curva para cada individuo y cada formulación disponible
auc <- zolmitriptan |>
...
# Trabajamos con una tabla auxiliar para no modificar la información original
aux <- auc |> 
  pivot_wider(names_from = "formulation", values_from = "auc") |> # Creamos dos columnas  para el AUC de la formulación de prueba y de referencia
  arrange(subj) |> # Ordenamos por sujeto
  mutate(Sujeto = as.character(subj), # Para asegurarnos que sale bien el resultado en gt
  ...
  ungroup() # Para que no duplique las observaciones en gt.

# Creamos una tabla con los títulos adecuados.
aux |>
...
En la pantalla imprimimos los estimadores de razón de  AUC, TMAX y CMAX.
...
La nueva tableta cumple con el criterio de bioequivalencia de la FDA debido a que los intervalos del 90% de confianza bootstrap de los estimadores de razón de AUC, TMAX y CMAX se encuentran dentro del intervalo (80%, 125%) requerido usando tanto el método de percentiles, como el método normal. A continuación presentamos los intervalos de confianza bootstrap de los estimadores de razón utilizando tanto el método de percentiles como el método normal.
```{r}
# Para reproducibilidad
set.seed(123)
# Realizamos las replicaciones bootsrap
...
```
Para tener mayor robustez de nuestros reusltados, presentamos gráficos qq y se observa la distribución bootstrap se comporta de manera similar a una distribucón normal. 

```{r, fig.width=15, fig.height=7}
# Realizamos gráficas de la distribución bootstrap y de los qq plots de cada estimador de razón.
dist_boot_auc  <- ggplot(dist_boot, aes(x=theta_auc)) + 
  geom_histogram(bins = 50)  + labs(title="Dist. bootstrap de estimador de razón de AUC", x = "Estimador de razón - AUC", y = "Conteo")
dist_boot_cmax <- ggplot(dist_boot, aes(x=theta_cmax)) + 
  geom_histogram(bins = 50) + labs(title="Dist. bootstrap de estimador de razón de CMAX", x = "Estimador de razón - CMAX", y = "Conteo")
dist_boot_tmax <- ggplot(dist_boot, aes(x=theta_tmax)) + 
  geom_histogram(bins = 50) + labs(title="Dist. bootstrap de estimador de razón de TMAX", x = "Estimador de razón - TMAX", y = "Conteo")
...
endutih_design <- tr_endutih |> 
  select(UPM, FAC_HOG, EST_DIS, P4_4, P4_1_6, P5_5, ENT) |> # solo nos quedamos con las columnas que utilizaremos para que las replicaciones se realicen rapidamente
  mutate(con_internet = if_else(P4_4==1,1,0), # Realizamos dummy para calcular de manera más sencilla las proporciones
         con_movil    = if_else(P4_1_6==1,1,0),
         con_fijo     = if_else(P5_5==1,1,0)) |>
  select(UPM, FAC_HOG, EST_DIS, ENT, con_internet, con_movil, con_fijo) |>
  as_survey_design(ids = UPM, weights = FAC_HOG, strata = EST_DIS) # Transformamos a objeto de diseño muestral

# Realizamos las replicaciones bootstrap 
...
```{r}
# Calculamos las estimaciones puntuales de las proporciones
...
# Imprimimos en la pantalla las estimaciones puntuales y los intervalos de confianza
...
Utilizaremos la proporción de hogares con acceso a internet. Del cálculo de las proporcines por estado podemos observar que:
...
# Añadimos un archivo con los nombres de los estados
estados <- read_delim("./datos/estados.txt", show_col_types = FALSE)
estado_internet_2 <- left_join(estado_internet, estados, by="ENT") |>select(nombre, inf, sup, prop_internet) |> arrange(nombre)# Creamos objeto nuevo para no tener problemas con el left_join

# Esta línea nos asegura que en el gráfico los estados se ordenen en orden alfabetico
estado_internet_2$nombre <- fct_reorder(estado_internet_2$nombre, estado_internet_2$prop_internet, .desc = FALSE)

# Realizamos una gráfica por estado de la estimación puntual de la proporción de hogares con acceso a internet y su intervalo de confianza
ggplot(estado_internet_2, aes(x = prop_internet, y = nombre, xmax = sup, xmin = inf)) + 
  geom_point(aes(colour = "Estimación puntual"), size = 2) + 
  geom_linerange(aes(colour = "Intervalo de confianza (95%)")) +
  geom_hline(yintercept = 0, colour = "gray") + 
  geom_text_repel(aes(label = round(prop_internet,3)), size=3) +
  labs(title="Proporción de hogares con acceso a internet en cada estado",
